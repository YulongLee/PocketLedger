...
Page({
  ...
  buildCalendar() { const days=[] as any[]; for(let i=1;i<=30;i++){const weekday=(i+1)%7; days.push({day:i,selected:i===15,dot:[1,2,8,10,15,17,23,25,29,30].includes(i),weekday})} const leading=1; for(let i=0;i<leading;i++) days.unshift({day:'',selected:false,dot:false}); this.setData({calendar:days}) },
  ...
  updateDaySummary(records:Tx[]) { const day=records.filter(r=>r.date==='2026-09-15'); const expense=day.filter(r=>r.type==='expense').reduce((s,r)=>s+r.amount,0); const income=day.filter(r=>r.type==='income').reduce((s,r)=>s+r.amount,0); this.setData({dayExpense:expense.toFixed(2),dayIncome:income.toFixed(2)}) },
